...
function buildWordmark() {
  ...
  const SZ = 320, TRACK = .28, PAD = 26;
  const m = cvs(4, 4).getContext('2d');
  m.font = '600 ' + SZ + 'px Wordmark, Onest, sans-serif';
  ...
  gl.forEach((g, i) => {
    ...
    x.font = '600 ' + SZ + 'px Wordmark, Onest, sans-serif';
...
/* ======================================================== 14 · booting */
const JOBS = [
  ['Reading the type', () => document.fonts && document.fonts.load('600 320px Onest')],
...
function start() {
  ...
  if (shot !== null) {                       /* deterministic states for review */
    ...
    preEl.classList.add('done');
    try { window.dispatchEvent(new CustomEvent('kage:loaded')); } catch(e){}
    setTimeout(() => {
      document.body.classList.remove('is-locked');
